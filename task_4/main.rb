...
class Control 
	...
	def add_route
		show_routes
		puts "Введите номер поезда, которому необходимо назначить маршрут"
		number = gets.chomp.to_i
		@trains[number].route == @routes
		#@station = routes.stations.first
		puts "К поезду №#{@trains[number]} добавлен маршрут от станции #{@routes.first} до станции #{@routes.last}"
	end
